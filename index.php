...
<body class="bg-light">
    ...
    <div class="container">
        <div class="row">
            ...
            <?php
            $room_result = select("SELECT * FROM `rooms` WHERE `status`=? AND `removed`=? ORDER BY `id` DESC LIMIT 3", 'ii', [1, 0]);
            while ($room_data = mysqli_fetch_assoc($room_result)) {
                ...
                while ($fea_row = mysqli_fetch_assoc($fea_q)) {
                    $features_data .= "<span class='badge rounded-pill bg-light text-dark text-wrap'>$fea_row[name]
                        </span>";
                }
                ...
                while ($fac_row = mysqli_fetch_assoc($fac_q)) {
                    $facilities_data .= "<span class='badge rounded-pill bg-light text-dark text-wrap'>$fac_row[name]
                        </span>";
                }
                ...
                $rating_q = "SELECT AVG(rating) AS `avg_rating` FROM `rate_review` WHERE `room_id`= '$room_data[id]' ORDER BY `sr_no` DESC LIMIT 20";
                        ...
                        <div class="card border-0 shadow" style="max-width: 350px; margin: auto;">
                            ...
                            <div class="card-body">
                                ...
                                <div class="guests mb-4">
                                    ...
                                    <span class="badge rounded-pill bg-light text-dark text-wrap">$room_data[adult] Adults
                                    </span>
                                    <span class="badge rounded-pill bg-light text-dark text-wrap">$room_data[children] Children
                                    </span>
                                </div>
                               $rating_data
    ...
    <div class="container mt-5">
        <div class="swiper swiper-testimonials">
            <div class="swiper-wrapper mb-5">
                <?php
                $review_q = "SELECT rr.* ,uc.name AS uname, uc.profile,r.name AS rname FROM `rate_review` rr INNER JOIN `user_cred` uc ON rr.user_id = uc.id INNER JOIN `rooms` r ON rr.room_id = r.id ORDER BY `sr_no` DESC LIMIT 6";
    ...
    if (isset($_GET['account_recovery'])) {
        ...
        $t_date = date("Y-m-d");
        $query = select("SELECT * FROM `user_cred` WHERE `email`=? AND `token`=? AND `t_expire`=? LIMIT 1", 'sss', [$data['email'], $data['token'], $t_date]);
