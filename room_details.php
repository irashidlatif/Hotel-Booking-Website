...
<body class="bg-light">
    ...
    $room_res = select("SELECT * FROM `rooms` WHERE `id`=? AND `status`=? AND `removed`=?", 'iii', [$data['id'], 1, 0]);
        ...
        <div class="row">
            ...
            <div class="col-12 mt-4 px-4">
                ...
                <div>
                    ...
                    <?php
                    $review_q = "SELECT rr.* ,uc.name AS uname, uc.profile,r.name AS rname FROM `rate_review` rr INNER JOIN `user_cred` uc ON rr.user_id = uc.id INNER JOIN `rooms` r ON rr.room_id = r.id 
