...
if (isset($_GET['email_confirmation'])) {
    ...
    $query = select("SELECT * FROM `user_cred` WHERE `email`= ? AND `token`=? LIMIT 1", 'ss', [$_GET['email'], $_GET['token']]);
    ...
    if (mysqli_num_rows($query) == 1) {
        ...
        if ($fetch['is_verified'] == 1) {
            ...
            $update = update("UPDATE `user_cred` SET `is_verified`= ? WHERE `id`= ? ", 'ii', [1, $fetch['id']]);
            if ($update) {
