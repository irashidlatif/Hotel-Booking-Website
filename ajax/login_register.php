...
function send_mail($uemail, $token, $type)
{
    ...
    $email->setFrom(SENDGRID_MAIL, SENDGRID_NAME);
...
if (isset($_POST['register'])) {
    ...
    //match password & confirm password field
    ...
    $u_exist = select("SELECT * FROM `user_cred` WHERE `email`=? OR `phonenum`=? LIMIT 1", 'ss', [$data['email'], $data['phonenum']]);
    ...
    //upload register data to server with password encript
    ...
    if (insert($query, 'sssssssss', $values)) {
        echo 1;
    } else {
        echo 'ins_failed';
    }
...
if (isset($_POST['login'])) {
    ...
    $u_exist = select("SELECT * FROM `user_cred` WHERE `email`=? OR `phonenum`=? LIMIT 1", 'ss', [$data['email_mob'], $data['email_mob']]);
...
if (isset($_POST['forgot_pass'])) {
    ...
    $u_exist = select("SELECT * FROM `user_cred` WHERE `email`=?  LIMIT 1", 's', [$data['email']]);
