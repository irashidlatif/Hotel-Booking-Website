...
if (isset($_GET['fetch_rooms'])) {
    ...
    // count no. of rooms & output variable to store rooms cards
    $count_rooms = 0;
    $output = "";
    ...
    $room_result = select("SELECT * FROM `rooms` WHERE `adult`>= ? AND `children`>= ? AND `status`=? AND `removed`=?", 'iiii', [$adults, $children, 1, 0]);
    while ($room_data = mysqli_fetch_assoc($room_result)) {
        ...
        if ($chk_avail['checkin'] != '' && $chk_avail['checkout'] != '') {
            ...
            $tb_fetch = mysqli_fetch_assoc(select($tb_query, 'siss', $values));
        ...
        $fac_q = mysqli_query($conn, "SELECT f.name, f.id FROM `facilities` f INNER JOIN `room_facilities` rfac ON f.id = rfac.facilities_id WHERE rfac.room_id = '$room_data[id]'");
        $facilities_data = "";
