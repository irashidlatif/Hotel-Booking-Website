...
if (isset($_POST['get_bookings'])) {
    ...
    ((bo.booking_status = 'booked' AND bo.arrival = 1) OR
    ...
    $limit_res = select($limit_query, 'sss', ["%$form_data[search]%", "%$form_data[search]%", "%$form_data[search]%"]);
    ...
    if ($total_rows == 0) {
        $output = json_encode(['table_data' => "<b>No such data!</b>", 'pagination' => '']);
        echo $output;
        exit;
    }
    ...
    while ($data = mysqli_fetch_assoc($limit_res)) {
        ...
        $table_data .= "
            <tr>
                ...
                <td>
                    <b>Amount:</b> ₨  $data[trans_amt] in cents
                    <br>
                    <b>Date:</b> $date
                </td>
                <td>
